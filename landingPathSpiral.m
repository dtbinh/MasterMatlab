...
%% Find Tmerk
%Start if where uav is far away
dL1 = sqrt((x0(1)-loiter1NED(1))^2+(x0(2)-loiter1NED(2))^2);
dL2 = sqrt((x0(1)-loiter2NED(1))^2+(x0(2)-loiter2NED(2))^2);
[dL,clWP]=min([dL1 dL2]);
if mod(clWP,2)==1
    OF = loiter1NED;
else
...
OS1 = [x0(1)-R_min*cos(x0(6)+pi/2);x0(2)-R_min*sin(x0(6)+pi/2);x0(3)];
OS2 = [x0(1)-R_min*cos(x0(6)-pi/2);x0(2)-R_min*sin(x0(6)-pi/2);x0(3)];
OS = [OS1 OS2];
DOS1 = sqrt((OS1(1)-OF(1))^2+(OS1(2)-OF(2))^2);
DOS2 = sqrt((OS2(1)-OF(1))^2+(OS2(2)-OF(2))^2);
[DOS,IOS]=min([DOS1 DOS2]);
cOSOFbx = OF(1);
cOSOFax = OS(1,IOS)-cOSOFbx;
cOSOFby = OF(2);
cOSOFay = OS(2,IOS)-cOSOFby;
RCsec = abs(Rl-R_min);
Tt = sqrt(RCsec^2/(cOSOFax^2+cOSOFay^2));
Tmerk1 = [cOSOFay*Tt+cOSOFbx;-cOSOFax*Tt+cOSOFby;loiter1NED(3)];
Tt = -sqrt(RCsec^2/(cOSOFax^2+cOSOFay^2));
Tmerk2 = [cOSOFay*Tt+cOSOFbx;-cOSOFax*Tt+cOSOFby;loiter1NED(3)];
Tnett1 = sqrt((WPNED(1,1)-Tmerk1(1))^2+(WPNED(2,1)-Tmerk1(2))^2);
Tnett2 = sqrt((WPNED(1,1)-Tmerk2(1))^2+(WPNED(2,1)-Tmerk2(2))^2);
Tmerk = [Tmerk1 Tmerk2];
[TN,Inett]=max([Tnett1 Tnett2]);
if Inett ==1
    Tt = sqrt(Rl^2/(cOSOFax^2+cOSOFay^2));
    PN = [cOSOFay*Tt+cOSOFbx;-cOSOFax*Tt+cOSOFby;OF(3)];
else
    Tt = -sqrt(Rl^2/(cOSOFax^2+cOSOFay^2));
    PN = [cOSOFay*Tt+cOSOFbx;-cOSOFax*Tt+cOSOFby;OF(3)];
end
TmerkOSbx = Tmerk(1,Inett);
TmerkOSax = OS(1,IOS)-TmerkOSbx;
TmerkOSby = Tmerk(2,Inett);
TmerkOSay = OS(2,IOS)-TmerkOSby;

% Might need to consided both directions: Correction must consider both
% directions

Tt = -sqrt(R_min^2/(cOSOFay^2+cOSOFax^2));


Pchi = [cOSOFay*Tt+OS(1,IOS);-cOSOFax*Tt+OS(2,IOS);loiter1NED(3)];
if x0(3)<WPNED(3,4)
    WPGlide = descent;
else
    WPGlide = -descent;
...
dPath = x0(1:3);
tt = 1;
theta0 = atan2(x0(2)-OS(2,IOS),x0(1)-OS(1,IOS));
theta = atan2(Pchi(2)-OS(2,IOS),Pchi(1)-OS(1,IOS));
if pipi(theta-theta0)== 0
    thetat = 0;
    No = 1;
else
    No = N;
...
turn = [OS(1,IOS) + R_min*cos(theta0+thetat);OS(2,IOS)+R_min*sin(theta0+thetat);ones(1,No)*x0(3)];
dPath = [dPath turn];
theta0 = atan2(PN(2)-OF(2),PN(1)-OF(1));
theta = atan2(WPNED(2,4)-OF(2),WPNED(1,4)-OF(1));
% thetat = 0:(pipi(theta-theta0))/(N-1):pipi(theta-theta0);
if mod(clWP,2)==1
    thetat = 0:-(pipi(theta-theta0))/(N-1):-pipi(theta-theta0);
else
...
dPNPchi = sqrt((PN(1)-Pchi(1))^2+(PN(2)-Pchi(2))^2);
if abs(atan2(WPNED(3,4)-x0(3),dPNPchi)*rad2deg)>abs(descent)
    turn = [OF(1) + Rl*cos(theta0+thetat);OF(2)+Rl*sin(theta0+thetat);ones(1,N)*(x0(3)+(dPNPchi)*tan(WPGlide*deg2rad))];
    dPath = [dPath turn];
    WPS0 = dPath(:,end);
    theta0 = atan2(WPS0(2)-OF(2),WPS0(1)-OF(1));
    if mod(clWP,2)==1
        thetat = 0:-(2*pi)/(N-1):-2*pi;
    else
        thetat = 0:(2*pi)/(N-1):2*pi;
    end
    xnn = OF(1)+Rl*cos(theta0+thetat(2));
    ynn = OF(2)+Rl*sin(theta0+thetat(2));
    n = 3;
    ...
    WPGlide = atan2(WPNED(3,4)-x0(3),dPNPchi);
    turn = [OF(1) + Rl*cos(theta0+thetat);OF(2)+Rl*sin(theta0+thetat);ones(1,N)*(x0(3)+(dPNPchi)*tan(WPGlide))];
    dPath = [dPath turn];
    correctH = true;
end

while(~correctH)
   if (abs(atan2(WPNED(3,4)-WPS1(3),D))*rad2deg<abs(descent))
       
       WPGlide = atan2(WPNED(3,4)-WPS1(3),D);
       
       
%        WPS0 = WPS1;
%        xnn = OF(1)+Rl*cos(theta0+thetat(n));
%        ynn = OF(2)+Rl*sin(theta0+thetat(n));
%        D = sqrt((xnn-WPS0(1))^2+(ynn-WPS0(2))^2);
%        znn = WPS0(3)+D*tan(WPGlide*deg2rad);
%        WPS1 = [xnn;ynn;znn];
%        tt = tt+1;
%        dPath(:,tt) = WPS1;
       ...
       theta0 = atan2(WPS1(2)-OF(2),WPS1(1)-OF(1));
       theta = atan2(WPNED(2,4)-OF(2),WPNED(1,4)-OF(1));
       if pipi(theta-theta0)==0
           thetat = 0;
           No = 1;
           ...
           No = N;
           if mod(clWP,2)==1
                if ((pipi(theta-pi)>=theta0 && sign(pipi(theta-pi))==sign(theta0)) || (theta0>theta &&sign(theta)==sign(theta0)))
                    thetat = 0:-(abs(pipi(theta-theta0))/(No-1)):-abs(pipi(theta-theta0));
                else
                    thetat = 0:-((2*pi-abs(pipi(theta-theta0)))/(No-1)):-(2*pi-abs(pipi(theta-theta0)));
                end
       ...
       turn = [OF(1) + Rl*cos(theta0+thetat);OF(2)+Rl*sin(theta0+thetat);ones(1,No)*(WPS1(3)+(D)*tan(WPGlide))];
       dPath = [dPath turn];
       tt = length(dPath);
       correctH = true;
   else
...
s = 0:0.01:1;
c = [cOSOFax*s + cOSOFbx;cOSOFay*s+cOSOFby];
tt = length(dPath)+1;
dPath(:,tt) = WPNED(:,3);
tt = tt+1;
dPath(:,tt) = WPNED(:,2);
...
plot(x0(2),x0(1),'o');
hold on;
% plot(Pchi(2),Pchi(1),'o');
plot(dPath(2,:),dPath(1,:),'-x');
plot(OF(2),OF(1),'o');
plot(c(2,:),c(1,:));
plot(Tmerk(2,Inett),Tmerk(1,Inett),'o');
plot(PN(2),PN(1),'x');
plot(WPNED(2,4),WPNED(1,4),'o');
plot(cl1NED(2,:),cl1NED(1,:));
plot(cl2NED(2,:),cl2NED(1,:));
plot(OS(2,IOS),OS(1,IOS),'x');
figure(6)
plot3(x0(2),x0(1),-x0(3),'o');
hold on;
plot3(dPath(2,:),dPath(1,:),-dPath(3,:));
% plot3(PN(2),PN(1),-PN(3),'o');
plot3(WPNED(2,4),WPNED(1,4),-WPNED(3,4),'x')
