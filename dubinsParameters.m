...
function dubinspath = dubinsParameters(start_node, end_node, R)
  ...
  if ell<2*R,
    ...
    % compute L1
    theta = acos((dot(crs,cre))/(norm(crs)*norm(cre)));
    L1 = norm(crs-cre) + R*(2*pi+(theta-pi/2)-(chis-pi/2))+R*(2*pi+(chie-pi/2)-(theta-pi/2));
    % compute L2
    ell = norm(cle-crs);
    theta = acos((dot(crs,cle))/(norm(crs)*norm(cle)));
    ...
    % compute L3
    ell = norm(cre-cls);
    theta = acos((dot(cls,cre))/(norm(cls)*norm(cre)));
    theta2 = acos(2*R/ell);
    if isreal(theta2)==0,
      L3 = 9999;
    else
    ...
    % compute L4
    theta = acos((dot(cls,cle))/(norm(cls)*norm(cle)));
    ...
    switch(idx),
        ...
        case 2,   
            ...
            lame = -1;
            ell = norm(ce-cs);
            theta = atan2(ce(2)-cs(2),ce(1)-cs(2));
            theta2 = theta - pi/2 *asin((2*R)/ell);
            q1 = rotz(theta2+pi/2)*(ce-cs);
            w1 = cs+R*rotz(-pi/2)*(ce-cs);
            w2 = ce + R*rotz(-pi/2)*(ce-cs);
        case 3,
            cs =cls;
            lams =-1;
            ce = cre;
            ...
            theta2 = theta-pi/2+asin((2*R)/ell);
            q1 = rotz(theta2+pi/2)*(ce-cs);
            w1 = cs+R*rotz(theta2)*(ce-cs);
            w2 = ce+R*rotz(theta2+pi)*(ce-cs);
         case 4,
            cs = cls;
            lams = -1;
            ce = cle;
